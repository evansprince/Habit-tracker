import { client, COMPLETIONS_COLLECTION_ID, database, DATABASE_ID, HABITS_COLLECTION_ID, RealTimeResponse } from "@/lib/appwrite";
...
export default function Index() {
  ...
  useEffect(() => {
    if (user) {
      const habitsChannel = `database.${DATABASE_ID}.collections.${HABITS_COLLECTION_ID}.documents`;
      const completionsChannel = `database.${DATABASE_ID}.collections.${COMPLETIONS_COLLECTION_ID}.documents`;

      const habitSubscription = client.subscribe(habitsChannel, (response: RealTimeResponse) => {
        if (
          response.events.includes("database.*.collections.*.create") ||
          response.events.includes("database.*.collections.*.update") ||
          response.events.includes("database.*.collections.*.delete")
        ) {
          fetchHabits();
        }
      });
      ...
      const completionsSubscription = client.subscribe(completionsChannel, (response: RealTimeResponse) => {
        if (response.events.includes("database.*.collections.*.create")) {
                ...
                <View style={styles.cardContent}>
                  ...
                  <View style={styles.cardFooter}>
                    <View style={styles.streakBadge}>
                      ...
                      <Text style={styles.streakText}>{`${habit.streak_count ?? 0} day streak`}</Text>
                    </View>
                    <View style={styles.frequencyBadge}>
                      <Text style={styles.frequencyText}>
                        {habit.frequency ? habit.frequency.charAt(0).toUpperCase() + habit.frequency.slice(1) : ""}
                      </Text>
                    </View>
                  </View>
                </View>
